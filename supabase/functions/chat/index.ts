    ...
    try {
      ...
      const openRouterRes = await fetch("https://openrouter.ai/api/v1/chat/completions", {
        ...
        headers: {
          ...
          "HTTP-Referer": Deno.env.get('OPENROUTER_HTTP_REFERER') ?? "https://logos.app",
          "X-Title": Deno.env.get('OPENROUTER_APP_NAME') ?? "Logos"
        },
        body: JSON.stringify({
      ...
      if (openRouterRes.ok) {
        ...
        if (openRouterData.choices && openRouterData.choices.length > 0) {
          reply = openRouterData.choices[0].message.content.trim();
        }
      } else {
      ...
      console.error('[chat] OpenRouter fetch error:', err);
      throw err;
    }
