...
/** Thinker names may not match exactly between the Constellation graph data
 *  and PHILOSOPHER_BIOS (e.g. "Nietzsche" vs "Friedrich Nietzsche") — fall
 *  back to substring matching like the other thinker lookups in this app. */
function findPhilosopherBio(name: string) {
  if (PHILOSOPHER_BIOS[name]) return PHILOSOPHER_BIOS[name];
  const query = name.toLowerCase();
  const key = Object.keys(PHILOSOPHER_BIOS).find((k) => {
    const candidate = k.toLowerCase();
    return query.includes(candidate) || candidate.includes(query);
  });
  return key ? PHILOSOPHER_BIOS[key] : undefined;
}
    ...
    return (
      <g>
        <motion.line
          ...
          transition={{
            duration: 1.5,
            delay: Math.min(0.5 + idx * 0.05, 2.0),
            ease: "easeOut",
          }}
        />
    ...
    return (
      <motion.div
        ...
        transition={{
          ...
          damping: 20,
          mass: 0.8,
          delay: Math.min(0.2 + idx * 0.05, 2.0),
        }}
      ...
      >
        <div className="relative group flex flex-col items-center justify-center">
          ...
          <div
            className={`w-3 h-3 rounded-full ${colorClass} ${glowClass} transition-transform duration-300 ${isHovered ? "scale-150" : "scale-100"}`}
          />

          <div
            className={`absolute top-5 whitespace-nowrap text-sm font-semibold transition-all duration-300 ${isHovered ? "text-white" : "text-white/60"}`}
          >
            {node.label}
          </div>
...
export const ConstellationMap: React.FC<ConstellationMapProps> = React.memo(
  ({ onClose, onFilterByThinker }) => {
    const { nodes, edges, isLoading, error } = useConstellation();
    ...
    const selectedNodeData = useMemo(() => {
      ...
      if (!node) return null;
      const bio = findPhilosopherBio(selectedNode);
      return { node, bio };
    }, [selectedNode, nodes]);
      ...
      >
        ...
        <div className="fixed top-[max(env(safe-area-inset-top),2rem)] left-8 right-8 flex justify-between items-center z-20 pointer-events-none">
          <div className="pointer-events-auto">
            <h2 className="text-white text-2xl font-serif italic tracking-tight mb-1">
              Knowledge Constellation
            </h2>
            <p className="text-white/40 text-xs tracking-widest uppercase font-mono">
              Tap a thinker to filter the Stream
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="relative w-full h-full min-w-[1000px] min-h-[800px] mt-24 mb-24 md:m-0">
          <svg className="absolute inset-0 w-full h-full z-0 pointer-events-none">
            {edges.map((edge, idx) => {
              ...
              const isHovered =
                hoveredSet !== null &&
                hoveredSet.has(edge.from) &&
                hoveredSet.has(edge.to);
                      ...
                      <p className="text-[10px] font-mono uppercase tracking-[0.15em] text-[#B5A48B] mt-1">
                        {selectedNodeData?.node.group
                          ? selectedNodeData.node.group
                              .charAt(0)
                              .toUpperCase() +
                            selectedNodeData.node.group.slice(1)
                          : "Philosopher"}
                      </p>
                    </div>
                  </div>
                ...
                {selectedNodeData?.bio && (
                  <div className="mb-6 space-y-4">
                    {selectedNodeData.bio.years && (
                      <div className="text-xs text-[#B5A48B] font-mono tracking-widest">
                        {selectedNodeData.bio.years}
                      </div>
                    )}
                    ...
                    {selectedNodeData.bio.keyWorks &&
                      selectedNodeData.bio.keyWorks.length > 0 && (
                        <div>
                          <span className="text-[10px] font-mono uppercase tracking-[0.15em] text-white/40 mb-2 block">
                            Key Works
                          </span>
