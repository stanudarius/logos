...
const ZenMode: React.FC<ZenModeProps> = ({ onClose, onSessionComplete }) => {
  ...
  // Fire the completion callback as its own effect, not from inside the
  // setTimeRemaining updater above (updaters must stay pure).
  useEffect(() => {
    if (isComplete) onSessionCompleteRef.current();
  }, [isComplete]);

  // Only one ZenMode instance is ever mounted at a time (gated by
  // isZenModeOpen), so it's safe to tear down the shared AudioContext here.
  useEffect(() => {
    return () => {
      if (sharedAudioContext && sharedAudioContext.state !== "closed") {
          ...
          {isRunning && !isComplete && (
            <div
              ...
              style={{
                ...
                height: 300,
                background:
                  "radial-gradient(circle, rgba(181,164,139,0.18) 0%, transparent 70%)",
              }}
            />
          )}
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            {isComplete ? (
              ...
              >
                ...
                <p className="text-lg font-serif italic text-[#5CB888]">
                  Session complete.
                </p>
              ...
              <>
                <span
                  className="text-5xl font-light text-white/90 tabular-nums tracking-tight"
                ...
                {isRunning && (
                  <div className="w-1.5 h-1.5 rounded-full bg-[#B5A48B]/60 mt-4 animate-breathe" />
                )}
              </>
            )}
          </div>
                ...
                >
                  {isRunning ? (
                    ...
                    <>
                      ...
                      <span>{hasStarted ? "Resume" : "Begin"}</span>
                    </>
                  )}
                </button>
                {hasStarted && (
