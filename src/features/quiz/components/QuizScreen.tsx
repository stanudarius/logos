...
export const QuizScreen: React.FC<{ onComplete: (answers: Record<string, string>) => void }> = ({ onComplete }) => {
  ...
  if (currentStep >= QUIZ_QUESTIONS.length) {
    return <QuizSummaryScreen answers={answers} onComplete={() => onComplete(answers)} />;
  }
    ...
    <div className="w-full h-[100dvh] bg-[#FAF8F3] flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div className={question.options.length > 5 ? "grid grid-cols-2 gap-3" : "flex flex-col gap-3"}>
            {question.options.map((opt, index) => (
              ...
              >
                <span className={question.options.length > 5 ? "text-[15px] font-medium leading-tight block" : "text-lg"}>{opt.label}</span>
              </button>
            ))}
          </div>
        </motion.div>
