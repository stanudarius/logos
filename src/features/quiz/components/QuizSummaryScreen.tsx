...
export const QuizSummaryScreen: React.FC<QuizSummaryProps> = ({
  ...
  const handleExplore = async () => {
    ...
    try {
      ...
      const success = trail ? await handleStartTrail(trail.id) : false;
      if (success) {
        setActiveTrailIndex(0);
        setPhoneTab("trail-view");
      }

      // Complete onboarding even if a trail cannot be loaded. The main feed
      // remains available and can recover independently.
      await onComplete();
    } finally {
