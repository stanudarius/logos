...
export function useVault(
  ...
  trackCardInteraction: (index: number, weight: number) => void,
) {
  ...
  useEffect(() => {
    ...
    const fetchVault = async () => {
      try {
        ...
        if (vault && isMounted) {
          setSavedVaultCards(
            vault.map((row) => row.card_data as SavedVaultCard),
          );
        }
      } catch (e) {
  ...
  const savedVaultCardIds = useMemo(
    () => new Set(savedVaultCards.map((c) => c.base_id || c.id)),
    [savedVaultCards],
  );

  const toggleSaveToVault = useCallback(
    async (card: FeedCard, index?: number) => {
      if (!session?.user || !card) return;
      const cardBaseId = card.base_id || card.id;
      const isSaved = savedVaultCardsRef.current.some(
        (c) => (c.base_id || c.id) === cardBaseId,
      );

      if (isSaved) {
        const indexInVault = savedVaultCardsRef.current.findIndex(
          (c) => (c.base_id || c.id) === cardBaseId,
        );
        const cardToRemove = savedVaultCardsRef.current[indexInVault];
        setSavedVaultCards((prev) =>
          prev.filter((c) => (c.base_id || c.id) !== cardBaseId),
        );
        try {
        ...
        if (index !== undefined) trackCardInteraction(index, 3);
        try {
          ...
          if (isAppMounted.current) {
            setSavedVaultCards((prev) =>
              prev.filter((c) => c.id !== vaultCard.id),
            );
          }
        }
      }
  ...
  const updateVaultCardAnnotation = useCallback(
    async (id: string, annotation: string) => {
      ...
      if (session?.user) {
        try {
          ...
          if (isAppMounted.current) {
            setSavedVaultCards((prev) =>
              prev.map((c) => (c.id === id ? cardToUpdate : c)),
            );
          }
        }
      }
  ...
  const assignToFolder = useCallback(
    async (id: string, folderName: string | undefined) => {
      const cardToUpdate = savedVaultCardsRef.current.find((c) => c.id === id);
      if (!cardToUpdate) return;
