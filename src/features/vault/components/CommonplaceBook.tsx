...
import { FolderPlus, Folder, Trash2, X } from "lucide-react";
import type { SavedVaultCard } from "@/src/features/vault/types";

const AnnotationInput = React.memo(
  ({
    id,
    initialValue,
    onUpdate,
...
const CommonplaceBook: React.FC<CommonplaceBookProps> = ({
  ...
  const folders = React.useMemo(
    () =>
      Array.from(
        new Set(cards.map((c) => c.user_folder).filter(Boolean)),
      ) as string[],
    [cards],
  );
  const displayCards = React.useMemo(
    () =>
      activeFolder
        ? orderedCards.filter((c) => c.user_folder === activeFolder)
        : orderedCards,
    [orderedCards, activeFolder],
  );
            ...
            <>
              ...
              {folders.map((f) => (
                ...
                >
                  ...
                  {f}
                  {activeFolder === f && <X className="w-2.5 h-2.5" />}
                </button>
              ))}
            </>
          )}
      ...
      <div className="flex-1 overflow-y-auto p-4 pb-24 relative z-10">
        {displayCards.length === 0 && activeFolder ? (
          <div className="flex flex-col items-center justify-center h-full space-y-4 opacity-70">
            ...
            <p className="text-sm font-serif italic text-[#1C1C1E]">
              This folder is empty.
            </p>
            <button
              onClick={() => {
                const firstUnassigned = orderedCards.find(
                  (c) => !c.user_folder,
                );
                if (firstUnassigned) {
      ...
      <AnimatePresence>
        {assigningCardId && (
          ...
          >
            ...
            >
              <h3 className="font-serif italic text-lg text-[#1C1C1E] mb-4">
                Assign to Folder
              </h3>

              {folders.length > 0 && (
                <div className="mb-5 space-y-2">
                  <p className="text-[10px] font-mono text-[#B5A48B] uppercase tracking-widest">
                    Existing Folders
                  </p>
