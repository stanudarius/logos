        ...
        <div className="flex-1 relative w-full h-full flex flex-col items-center justify-center overflow-hidden">
          ...
          <AnimatePresence>
            {isChatOpen && (
              ...
              >
                {(() => {
                  const activeCard = phoneTab === "trail-view" ? activeTrailCards[activeTrailIndex] : feedCards[activeExploreIndex];
                  return (
                    <SocraticChat 
                      philosopher={activeCard?.philosopher || ""}
                      topic={activeCard?.topic || ""}
                      essayContext={activeCard?.presentation?.reading_parts?.map((p: any) => p.text).join(" ") || ""}
