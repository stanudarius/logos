...
export const getRandomInterstitial = (): FeedCard => {
  if (interstitialDeck.length === 0) {
    interstitialDeck = [...INTERSTITIAL_CARDS];
    for (let i = interstitialDeck.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [interstitialDeck[i], interstitialDeck[j]] = [interstitialDeck[j], interstitialDeck[i]];
    }
  }
